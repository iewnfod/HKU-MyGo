...
export default function FloatingSearchBar({navigator} : {navigator: Navigator}) {
	...
	const [selectOptionIndex, setSelectOptionIndex] = useState<number>(0);
						...
						<div className="flex flex-col gap-1 w-full p-2">
							{
								options.map((option, index) => (
									<Card
										className="w-full select-none shadow-none rounded-lg"
										variant={selectOptionIndex === index ? "secondary" : "transparent"}
										key={index}
									>
										<Card.Content>
											<div className="flex flex-col w-full">
												<p className="text-sm">{option.name}</p>
												<p className="text-xs text-default-500 overflow-hidden text-ellipsis text-wrap">
													{option.description}
												</p>
											</div>
										</Card.Content>
